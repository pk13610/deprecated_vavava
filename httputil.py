...
class HttpUtil(object):
    ...
    def fetch(self, url, handler, post_data=None):
        ...
        while data and handler.handle(data):
            data = self._request(url, post_data, handler)
...
class DownloadStream:
    """ download a stream by duration
        duration --> s
    """
    def __init__(self, name, duration, mode="w"):
        self.fp = open(name, mode)
    ...
    def handle(self, data):
        self.fp.write(data)
        self.duration = self.duration-time.clock()
...
def test_fetch():
    ...
    handle = DownloadStream('/Users/pk/Downloads/tmp.flv', duration=10)
